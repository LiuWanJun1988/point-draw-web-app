...
import 'package:pointdraw/point_draw_models/utilities/ui_parameters.dart' show controlPointSize;
...
abstract class PointDrawObject extends PointDrawStateNotifier{
  ...
  PointDrawObject.fromDocument(this.mode, DocumentSnapshot<Map<String, dynamic>?> snapshot, {required this.key}){
    Map<String, dynamic>? data = snapshot.data();
    if(data != null){
      List<Map<String, double>> cpData = [for(Map o in data[controlPointsKey]) Map.from(o)];
      List<Map<String, double>> rcpData = [for(Map o in data[restrictedControlPointsKey]) Map.from(o)];
      List<Map<String, double>> dcpData = [for(Map o in data[dataControlPointsKey]) Map.from(o)];
      for(int i = 0; i < cpData.length; i++){
        points += [Offset(cpData[i][xCoordinateKey]!, cpData[i][yCoordinateKey]!)];
      }
      for(int i = 0; i < rcpData.length; i++){
        rPoints += [Offset(rcpData[i][xCoordinateKey]!, rcpData[i][yCoordinateKey]!)];
      }
      for(int i = 0; i < dcpData.length; i++){
        dPoints += [Offset(dcpData[i][xCoordinateKey]!, dcpData[i][yCoordinateKey]!)];
      }
      sPaint.strokeWidth = data[strokeWidthKey];
      var strokeColor = data[strokeColorKey];
      sPaint.color = Color.fromARGB(strokeColor[0] as int, strokeColor[1] as int, strokeColor[2] as int, strokeColor[3] as int);
      var fillColor = data[fillColorKey];
      fPaint.color = Color.fromARGB(fillColor[0] as int, fillColor[1] as int, fillColor[2] as int, fillColor[3] as int);
      shaderParam = data[shaderKey] != null ? ShaderParameters.fromData(data[shaderKey]) : null;
  ...
  PointDrawObject.from(PointDrawObject object, {this.mode = EditingMode.object, required this.key, Offset displacement = const Offset(5, 5)}){
    mode = object.mode;
    points = List<Offset>.generate(object.points.length, (ind) => object.points[ind] + displacement);
    rPoints = List<Offset>.generate(object.rPoints.length, (ind) => object.rPoints[ind] + displacement);
    dPoints = List<Offset>.generate(object.dPoints.length, (ind) => object.dPoints[ind] + displacement);
    sPaint = Paint()
    ...
    boundingRect = Rect.fromPoints(object.boundingRect.topLeft + displacement, object.boundingRect.bottomRight + displacement);
    fPaint = Paint()
  ...
  Map<String, dynamic> toJson({bool parsePoints = true}){
    Map<String, dynamic> data = {
      ...
      filledKey: filled,
      fillColorKey: [fPaint.color.alpha, fPaint.color.red, fPaint.color.green, fPaint.color.blue],
      outlinedKey: outlined,
      strokeColorKey: [sPaint.color.alpha, sPaint.color.red, sPaint.color.green, sPaint.color.blue],
    ...
    if(parsePoints){
      List<Map<String, double>> controlPoints = [for(Offset p in points) {xCoordinateKey: p.dx, yCoordinateKey: p.dy}];
      List<Map<String, double>> restrictedPoints = [for(Offset p in rPoints) {xCoordinateKey: p.dx, yCoordinateKey: p.dy}];
      List<Map<String, double>> dataPoints = [for(Offset p in dPoints) {xCoordinateKey: p.dx, yCoordinateKey: p.dy}];
  ...
  void toObject(Map<String, dynamic> data, {bool parsePoints = true}){
    ...
    filled = data[filledKey];
    sPaint = Paint()
      ..color = Color.fromARGB(data[strokeColorKey][0], data[strokeColorKey][1], data[strokeColorKey][2], data[strokeColorKey][3])
      ..strokeWidth = data[strokeWidthKey]
      ..style = PaintingStyle.stroke;
    fPaint = Paint()
      ..color = Color.fromARGB(data[fillColorKey][0], data[fillColorKey][1], data[fillColorKey][2], data[fillColorKey][3])
      ..style = PaintingStyle.fill;
    shaderParam = data[shaderKey] != null ? ShaderParameters.fromData(data[shaderKey]) : null;
    if(shaderParam != null){
      fPaint.shader = shaderParam!.build();
    }
    List<Map<String, dynamic>> clipObjects = [for(Map obj in data[clipKey]) Map.from(obj)];
  ...
  void updateStrokePaint({double? strokeWidth, Color? color, bool? isOutlined, bool notify = true}){
    if(isOutlined != null){
      outlined = isOutlined;
    }
    if(strokeWidth != null){
  ...
  List<Offset?> get sPoints => [shaderParam?.center, shaderParam?.from, shaderParam?.to, shaderParam?.focal];

  void updateFillPaint(Matrix4 zoomTransform, {ShaderParameters? shaderParameters, bool? useShader, Color? color, bool? isFilled, bool notify = true}){
    if(isFilled != null){
      filled = isFilled;
    }
    if(useShader != null){
      shaderParam ??= ShaderParameters(boundingRect: boundingRect);
      fPaint.shader = useShader ? shaderParam!.build(zoomTransform: zoomTransform) : null;
    }
    if(shaderParameters != null){
      shaderParam = shaderParameters;
      fPaint.shader = shaderParam!.build(boundingRect: boundingRect, zoomTransform: zoomTransform);
    }
    if(color != null){
      fPaint.color = color;
    }
  ...
  ShaderType get shaderType => shaderParam != null ? shaderParam!.type : ShaderType.linear;

  void updateShaderParams(Matrix4 zoomTransform, {
    ...
    if(removeIndex != null){
      ...
      shaderParam?.updateShader(
          ...
          stopsList: stopsList,
          mode: mode,
          r: radialMultiplier != null ? (radialMultiplier * boundingRect.shortestSide) : null,
  ...
  @override
  void updateObject(Function(PointDrawObject) updatingCall, {bool executeAll = true, List<StateSetter> exclusion = const []}){
  ...
  @mustCallSuper
  PointDrawObject moveControlPoint(Offset newPosition, int index, {Map<String, dynamic>? args}){
    points[index] = newPosition;
    return this;
  }

  @mustCallSuper
  PointDrawObject updateRDSCPWhenCPMoved(Matrix4 zoomTransform, {Map<String, dynamic> args = const {}}){
  ...
  PointDrawObject moveRestrictedControlPoint(Offset localPosition, int index, {Map<String, dynamic>? args}){
    return rPoints.isNotEmpty ? throw UnimplementedError(runtimeType.toString() + " did not implement moveRestrictedControlPoint") : this;
  }

  PointDrawObject moveDataControlPoint(Offset newPosition, int index, {Map<String, dynamic>? args}){
    dPoints[index] = newPosition;
    return this;
  }

  PointDrawObject transformByTranslate(double dx, double dy, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    points = groupControlPoints ?? List<Offset>.generate(points.length, (ind) => points[ind] + Offset(dx, dy));
    rPoints = List<Offset>.generate(rPoints.length, (ind) => rPoints[ind] + Offset(dx, dy));
    dPoints = List<Offset>.generate(dPoints.length, (ind) => dPoints[ind] + Offset(dx, dy));
    Map<Path, PointDrawObject> transformedClips = {};
    for(Path clipPath in clips.keys){
      transformedClips[clipPath.transform(Matrix4.translationValues(dx, dy, 0).storage)] = clips[clipPath]!.transformByTranslate(dx, dy, zoomTransform, groupControlPoints: groupControlPoints);
    }
  ...
  PointDrawObject transformByRotate(Offset center, double angle, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    ...
    dPoints = List<Offset>.generate(dPoints.length, (ind) => rotate(dPoints[ind], center, angle));
    Map<Path, PointDrawObject> transformedClips = {};
    for(Path clipPath in clips.keys){
      transformedClips[clipPath.transform(rotateZAbout(angle, center).storage)] = clips[clipPath]!.transformByRotate(center, angle, zoomTransform, groupControlPoints: groupControlPoints);
    }
    clips = transformedClips;
  ...
  PointDrawObject transformByHorizontalScale(Offset stationary, double scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    ...
    rPoints = List<Offset>.generate(rPoints.length, (ind) => Offset(stationary.dx + (rPoints[ind].dx - stationary.dx) * scaleFactor, rPoints[ind].dy));
    dPoints = List<Offset>.generate(dPoints.length, (ind) => Offset(stationary.dx + (dPoints[ind].dx - stationary.dx) * scaleFactor, dPoints[ind].dy));
    Map<Path, PointDrawObject> transformedClips = {};
    for(Path clipPath in clips.keys){
      transformedClips[clipPath.transform(scalingX(scaleFactor, stationary).storage)] = clips[clipPath]!.transformByHorizontalScale(stationary, scaleFactor, zoomTransform, groupControlPoints: groupControlPoints);
    }
    clips = transformedClips;
  ...
  PointDrawObject transformByVerticalScale(Offset stationary, double scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    ...
    rPoints = List<Offset>.generate(rPoints.length, (ind) => Offset(rPoints[ind].dx, stationary.dy + (rPoints[ind].dy - stationary.dy) * scaleFactor));
    dPoints = List<Offset>.generate(dPoints.length, (ind) => Offset(dPoints[ind].dx, stationary.dy + (dPoints[ind].dy - stationary.dy) * scaleFactor));
    Map<Path, PointDrawObject> transformedClips = {};
    for(Path clipPath in clips.keys){
      transformedClips[clipPath.transform(scalingY(scaleFactor, stationary).storage)] = clips[clipPath]!.transformByVerticalScale(stationary, scaleFactor, zoomTransform, groupControlPoints: groupControlPoints);
    }
    clips = transformedClips;
  ...
  PointDrawObject transformByScale(Offset stationary, Offset scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    ...
    rPoints = List<Offset>.generate(rPoints.length, (ind) => Offset(stationary.dx + (rPoints[ind].dx - stationary.dx) * scaleFactor.dx, stationary.dy + (rPoints[ind].dy - stationary.dy) * scaleFactor.dy));
    dPoints = List<Offset>.generate(dPoints.length, (ind) => Offset(stationary.dx + (dPoints[ind].dx - stationary.dx) * scaleFactor.dx, stationary.dy + (dPoints[ind].dy - stationary.dy) * scaleFactor.dy));
    Map<Path, PointDrawObject> transformedClips = {};
    for(Path clipPath in clips.keys){
      transformedClips[clipPath.transform(scalingXY(scaleFactor, stationary).storage)] = clips[clipPath]!.transformByScale(stationary, scaleFactor, zoomTransform, groupControlPoints: groupControlPoints);
    }
    clips = transformedClips;
  ...
  void flipHorizontal(Matrix4 zoomTransform, {Offset? center}){
    if(boundingRect != Rect.zero){
      ...
      points = List<Offset>.generate(points.length, (ind) => Offset(center!.dx + (center.dx - points[ind].dx), points[ind].dy));
      rPoints = List<Offset>.generate(rPoints.length, (ind) => Offset(center!.dx + (center.dx - rPoints[ind].dx), rPoints[ind].dy));
      dPoints = List<Offset>.generate(dPoints.length, (ind) => Offset(center!.dx + (center.dx - dPoints[ind].dx), dPoints[ind].dy));
      Map<Path, PointDrawObject> transformedClips = {};
      for(Path clipPath in clips.keys){
        PointDrawObject obj = clips[clipPath]!;
        obj.flipHorizontal(zoomTransform);
        transformedClips[clipPath.transform(horizontalFlip(center).storage)] = obj;
      }
      clips = transformedClips;
  ...
  void flipVertical(Matrix4 zoomTransform, {Offset? center}){
    if(boundingRect != Rect.zero){
      center ??= boundingRect.center;
      points = List<Offset>.generate(points.length, (ind) => Offset(points[ind].dx, center!.dy + (center.dy - points[ind].dy)));
      rPoints = List<Offset>.generate(rPoints.length, (ind) => Offset(rPoints[ind].dx, center!.dy + (center.dy - rPoints[ind].dy)));
      dPoints = List<Offset>.generate(dPoints.length, (ind) => Offset(dPoints[ind].dx, center!.dy + (center.dy - dPoints[ind].dy)));
      Map<Path, PointDrawObject> transformedClips = {};
      for(Path clipPath in clips.keys){
        PointDrawObject obj = clips[clipPath]!;
        obj.flipVertical(zoomTransform);
        transformedClips[clipPath.transform(verticalFlip(center).storage)] = obj;
      }
      clips = transformedClips;
...
class FreeDraw extends  PointDrawPath{
  ...
  FreeDraw.fromDocument(
      DocumentSnapshot<Map<String, dynamic>> snapshot, {required ObjectKey key}) : super.fromDocument( snapshot,
    ...
    supplementaryPropertiesModifiers.addAll([getReadyToShiftToggleButton, getToggleCloseButton, getSplineEffectMenuButton, getToggleShowControlPoints]);
  }
  ...
  FreeDraw.from(FreeDraw object, {required ObjectKey key, Offset displacement = const Offset(5, 5)}) : super.from(object, mode: EditingMode.freeDraw, key: key){
    closed = object.closed;
    _drawEnd = object.drawEnd;
    coarseness = object.coarseness;
    tension = object.tension;
    splinePath = SplinePath.generate(
      List<Offset>.generate(object.splinePath.points.length, (index) => object.splinePath.points[index] + displacement),
      tension: tension,
    );
    supplementaryPropertiesModifiers.addAll([getReadyToShiftToggleButton, getToggleCloseButton, getSplineEffectMenuButton, getToggleShowControlPoints]);
  }
  ...
  @override
  void updateStrokePaint({double? strokeWidth, Color? color, bool? isOutlined, bool notify = true, Matrix4? zoomTransform}){
    if(strokeWidth != null){
      sPaint.strokeWidth = strokeWidth;
    }
    if(color != null){
      sPaint.color = color;
      updateFillPaint(zoomTransform ?? Matrix4.identity(), color: color, notify: false);
    }
    if(notify){
      notifyListeners();
    }
  ...
  void drawCriticalPoints(Canvas canvas){
    if(splinePath.criticalPointIndices.isNotEmpty){
      for(int ind in splinePath.criticalPointIndices){
        canvas.drawOval(
          Rect.fromCenter(center: splinePath.points[ind], width: 4.0, height: 4.0),
          Paint()
            ..color = Colors.red
            ..strokeWidth = 3.0
            ..style = PaintingStyle.stroke,
  ...
  @override
  FreeDraw updateRDSCPWhenCPMoved(Matrix4 zoomTransform, {Map<String, dynamic> args = const {}}){
  ...
  void updateEffectsParams({double? maxWidth, double? variance, double? endWidth, double? pointsGapCoefficient, bool notify = true}){
    if(maxWidth != null){
      effectsParams.maxWidth = maxWidth;
      effectsParams.variance = min(effectsParams.variance, effectsParams.maxWidth);
      effectsParams.endWidth = min(effectsParams.endWidth, effectsParams.maxWidth);
    }
    if(variance != null){
  ...
  void regenerateSpline({bool filter = false, bool computeMetric = false}){
    splinePath.enGenerate(effect: effectsParams.type, effectsParams: effectsParams, filter: filter, computeMetric: computeMetric);
    notifyListeners();
  }
  ...
  FreeDraw duplicate({Offset? center}){
    if(center != null){
      return FreeDraw.from(this, displacement: (center - boundingRect.center), key: ObjectKey("FreeDraw: ${generateAutoID()}"));
    } else {
      return FreeDraw.from(this, key: ObjectKey("FreeDraw: ${generateAutoID()}"));
    }
        ...
        return [
          for(SplineEffects effect in SplineEffects.values)
            PopupMenuItem(
              ...
              child: Text(effect.name, style: const TextStyle(fontSize: 14, color: Colors.white)),
            ),
        ];
      },
      onSelected: (SplineEffects val){
        effectsParams.type = val;
        splinePath.enGenerate(effect: val, filter: false, effectsParams: effectsParams);
        notifyListeners();
      },
      child: Material(
        ...
        child: Container(
          ...
          child: Text(effectsParams.type.name, style: const TextStyle(fontSize: 14, color: Colors.white))),
      ),
      tooltip: "Effects",
  ...
  Widget getToggleShowControlPoints(){
    return ActionButton(
      mode,
      showControlPoints,
      displayWidget: const ShowControlPointsIcon(widthSize: 28,),
      onPressed: (){
        showControlPoints = !showControlPoints;
        if(showControlPoints){
          points = splinePath.filteredPoints.isNotEmpty ? splinePath.filteredPoints : splinePath.points;
        } else {
          points = [];
        }
        notifyListeners();
...
abstract class PointDrawPath extends PointDrawObject{
  ...
  PointDrawPath({EditingMode mode = EditingMode.path, required ObjectKey key}) : super(mode: mode, key: key);

  PointDrawPath.fromDocument(
  ...
  PointDrawPath.from(PointDrawPath object, {mode = EditingMode.path, Offset displacement = const Offset(5, 5), required ObjectKey key}) : super.from(object, displacement: displacement, mode: mode, key : key);
  ...
  List<Offset> getAnimatedPoints(double ticker){
    List<Offset> animatedPoints = List<Offset>.generate(points.length, (ind){
      if(animationParams.animatedControlPoints.keys.contains(ind)){
        return points[ind] + animationParams.animatedControlPoints[ind]!.begin! * ticker + animationParams.animatedControlPoints[ind]!.end! * (1 - ticker);
      } else {
        return points[ind];
      }
    });
...
class PointDrawDirectedLine extends PointDrawPath{
  ...
  PointDrawDirectedLine({required ObjectKey key}) : super(mode: EditingMode.directedLine, key: key);
  ...
  PointDrawDirectedLine duplicate({Offset? center}){
    return PointDrawDirectedLine.from(this, key: ObjectKey("DirectedLine:"+generateAutoID()));
  }
...
class PointDrawCurvedDirectedLine extends PointDrawPath{
  ...
  PointDrawCurvedDirectedLine.fromDocument(
      ...
      ) : super.fromDocument(snapshot, mode: EditingMode.curvedDirectedLine, key: key);

  PointDrawCurvedDirectedLine({required ObjectKey key}) : super(mode: EditingMode.curvedDirectedLine, key: key);
  ...
  Path getAnimatedPath(double ticker){
    ...
    if(points.length == 2){
      ...
      animatedCurvedDirectedLine.cubicTo(controlPoint1.dx, controlPoint1.dy, controlPoint2.dx, controlPoint2.dy, pointer.dx, pointer.dy);
  ...
  PointDrawCurvedDirectedLine duplicate({Offset? center}){
    return PointDrawCurvedDirectedLine.from(this, key: ObjectKey("CurvedDirectedLine:"+generateAutoID()));
  }
...
class PointDrawGroup extends PointDrawPath{
  ...
  PointDrawGroup.fromDocument(
      DocumentSnapshot<Map<String, dynamic>> snapshot, {required ObjectKey key}
  ) : super.fromDocument(snapshot, mode: EditingMode.group, key: key);

  PointDrawGroup(this.group, {required ObjectKey key}) : super(mode: EditingMode.group, key: key){
    Path rect = Path();
  ...
  PointDrawGroup.from(PointDrawGroup object, {Offset displacement = const Offset(5, 5), required ObjectKey key}) : super.from(object, displacement: displacement, mode: EditingMode.group, key: key){
  ...
  void toObject(Map<String, dynamic> data, {parsePoints = true}){
    ...
    group = List.generate(groupData.length, (ind) => getNewPointDrawObject(getEditingMode(groupData[ind][editingModeKey]))..toObject(groupData[ind]));
    ...
    for(PointDrawObject pdo in group){
      if(pdo is PointDrawPath){
        if(clips.isNotEmpty){
          ...
          path.addPath(pdo.draw(canvas, ticker, zoomTransform: zoomTransform), Offset.zero);
          canvas.restore();
        } else {
          path.addPath(pdo.draw(canvas, ticker, zoomTransform: zoomTransform), Offset.zero);
        }
        ...
        if(clips.isNotEmpty){
          ...
          pdo.draw(canvas, ticker, zoomTransform: zoomTransform, drawText: false);
          canvas.restore();
        } else {
          pdo.draw(canvas, ticker, zoomTransform: zoomTransform, drawText: false);
        }
  ...
  void updateStrokePaint({double? strokeWidth, Color? color, bool? isOutlined, bool notify = true}){
    for(PointDrawObject pdo in group){
      pdo.updateStrokePaint(strokeWidth: strokeWidth, color: color, isOutlined: isOutlined, notify: false);
    }
    super.updateStrokePaint(strokeWidth: strokeWidth, color: color, isOutlined: isOutlined, notify: false);
    if(notify){
      notifyListeners();
    }
  }
  ...
  @override
  void updateFillPaint(Matrix4 zoomTransform, {bool? useShader, ShaderParameters? shaderParameters, Color? color, bool? isFilled, bool notify = true}){
    for(PointDrawObject pdo in group){
      pdo.updateFillPaint(zoomTransform, useShader: useShader, shaderParameters: shaderParameters, color: color, isFilled: isFilled, notify: false);
    }
    super.updateFillPaint(zoomTransform, shaderParameters: shaderParameters, useShader: useShader, color: color, isFilled: isFilled, notify: false);
    if(notify){
      notifyListeners();
    }
  }
      ...
      for(int k = 0; k < group.length; k++){
        ...
        if(sumOfControlPoints + pdo.points.length >= cpIndex){
          if(pdo.mode != EditingMode.group) {
            ...
            Map<String, dynamic> result = groupPDO.curveFinderFunction(cpIndex:cpIndex - sumOfControlPoints - 1);
      ...
      for(int k = 0; k < group.length; k++){
        ...
        if(sumOfRestrictedPoints + pdo.rPoints.length >= rcpIndex){
          if(pdo.mode != EditingMode.group) {
            ...
            PointDrawGroup groupPDO = pdo as PointDrawGroup;
            Map<String, dynamic> result = groupPDO.curveFinderFunction(rcpIndex:rcpIndex - sumOfRestrictedPoints - 1);
            result["path"] = "$k/" + result["path"];
            result["from"] += (sumOfControlPoints + 1);
            result["to"] += (sumOfControlPoints + 1);
            result["restricted_from"] += (sumOfRestrictedPoints + 1);
      ...
      for(int k = 0; k < group.length; k++){
        ...
        if(sumOfRestrictedPoints + pdo.dPoints.length >= dcpIndex){
          if(pdo.mode != EditingMode.group) {
            ...
            PointDrawGroup groupPDO = pdo as PointDrawGroup;
            Map<String, dynamic> result = groupPDO.curveFinderFunction(dcpIndex:dcpIndex - sumOfDataPoints - 1);
            result["path"] = "$k/" + result["path"];
            result["from"] += (sumOfControlPoints + 1);
            result["to"] += (sumOfControlPoints + 1);
            result["restricted_from"] += (sumOfRestrictedPoints + 1);
  ...
  Map<String, dynamic> Function({num? cpIndex, num? rcpIndex}) curveFinderGetter(List<PointDrawObject> objects){
  ...
  @override
  PointDrawGroup moveControlPoint(Offset newPosition, int index, {Map<String, dynamic>? args}){
    ...
    for(int i = 0; i < group.length; i++){
      ...
      if(index < cpFrom + cpLen ){
        group[i] = group[i].moveControlPoint(newPosition, index - cpFrom, args: args);
  ...
  @override
  PointDrawGroup moveRestrictedControlPoint(Offset localPosition, int index, {Map<String, dynamic>? args}){
    int from = 0;
    int len;
    for(int i = 0; i < group.length; i++){
      len = group[i].rPoints.length;
      if(index < from + len){
        group[i] = group[i].moveRestrictedControlPoint(localPosition, index - from, args: args);
  ...
  @override
  PointDrawGroup moveDataControlPoint(Offset newPosition, int index, {Map<String, dynamic>? args}){
    ...
    for(int i = 0; i < group.length; i++){
      ...
      rcpLen = group[i].rPoints.length;
      if(index < from + len){
        group[i] = group[i].moveDataControlPoint(newPosition, index - from, args: args);
        for(int j = rcpFrom; j < rcpFrom + rcpLen; j++){
          rPoints[j] = group[i].rPoints[j - rcpFrom];
        }
  ...
  PointDrawGroup duplicate({Offset? center}){
    if(center != null){
      ...
      return PointDrawGroup.from(this, displacement: displacement, key: ObjectKey("Group:" + generateAutoID()));
    } else {
      return PointDrawGroup.from(this, key: ObjectKey("Group:" + generateAutoID()));
    }
  }
...
class PointDrawText extends PointDrawObject {
  ...
  PointDrawText.fromDocument(
      DocumentSnapshot<Map<String, dynamic>> snapshot, {required ObjectKey key}) : super.fromDocument(EditingMode.text, snapshot, key: key){
    fPaint.color = Colors.white;
    fPaint.shader = null;
    sPaint.strokeJoin = StrokeJoin.miter;
    filled = true;
    supplementaryPropertiesModifiers.addAll([
      getIncrementFontButton, getDecrementFontButton, getAlignLeftButton, getAlignCenterButton, getAlignRightButton, getToggleBoldButton, getToggleRoundedRectangleBorderButton, getFontFamilySelection
    ]);
  }
  ...
  PointDrawText.from(PointDrawText object, {Offset displacement = const Offset(5, 5), required ObjectKey key}) : super.from(object, displacement: displacement, mode: EditingMode.text, key: key){
  ...
  Rect drawBorder(Canvas canvas, {Matrix4? zoomTransform}){
    ...
    if(points.length == 3 && rPoints.isNotEmpty){
      ...
      roundedRectPath.addRRect(
          RRect.fromRectAndRadius(Rect.fromPoints(points.first, points[1]),
              Radius.circular(radius ?? 0)));
    }
    Path arrowPath = Path()..addPolygon([
  ...
  @override
  void draw(Canvas canvas, double ticker, {Matrix4? zoomTransform, bool drawText = true}){
    Rect? borderRect;
    ...
    if(points.isNotEmpty){
      ...
      bottomRight = matrixApply(zoomTransform, points.first + Offset(width, height));
      if(drawText){
        TextPainter textPainter = buildTextPainter(factor: zoomTransform.storage[0]);
        Offset adjustment;
        if(radius != null){
      ...
      boundingRect = borderRect ?? getTextEditingArea(topLeft, bottomRight).getBounds();
    }
  }

  TextPainter buildTextPainter({factor = 1.0}){
    TextStyle textStyle = TextStyle(color: sPaint.color, fontSize: fontSize * factor, fontFamily: fontFamily, fontWeight: weight);
  ...
  UserInputWidget getInputController(TextEditingController controller, double zoomFactor){
  ...
  @override
  PointDrawText moveControlPoint(Offset newPosition, int index, {Map<String, dynamic>? args}){
    super.moveControlPoint(newPosition, index, args: args);
    if(roundedRectangleChatBoxBorder){
  ...
  @override
  PointDrawText updateRDSCPWhenCPMoved(Matrix4 zoomTransform, {Map<String, dynamic> args = const {}}){
    super.updateRDSCPWhenCPMoved(zoomTransform);
    radius = min(
        radius ?? 0,
        (points.first - points.last).dy.abs() / 2
    );
  ...
  @override
  PointDrawText moveRestrictedControlPoint(Offset localPosition, int index, {Map<String, dynamic>? args}){
    assert(roundedRectangleChatBoxBorder, "Text box should only enable restricted control point when using rounded rectangle chat box border");
    if(index == 0){
      ...
      Rect rect = Rect.fromPoints(points.first, points[1]);
      gap = max(min(rect.bottomRight.dx - localPosition.dx, rect.width - radius! - 10), radius! + _arrowGap);
      Offset computedPoint = Offset(rect.bottomRight.dx - gap, rect.bottomRight.dy);
      if(boundingRect.inflate(controlPointSize).contains(computedPoint)){
        rPoints[index] = computedPoint;
      }
    }
  ...
  @override
  PointDrawText transformByRotate(Offset center, double angle, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    if(roundedRectangleChatBoxBorder){
      points = groupControlPoints ?? List<Offset>.generate(points.length, (ind) => rotate(points[ind], center, angle));
      Rect rect = Rect.fromPoints(points.first, points.last);
      radius = max(5.0, (rect.topLeft - rPoints.first).distance);
      gap = max(radius! + _arrowGap, min(rect.width - radius! - _arrowGap, gap));
      rPoints = [rect.topLeft + Offset.fromDirection(pi / 2, radius!), rect.bottomRight + Offset(-gap, 0)];
      shaderParam = shaderParam?.transformByRotate(center, angle);
      fPaint.shader = fPaint.shader != null ? shaderParam?.build(boundingRect: boundingRect, zoomTransform: zoomTransform) : null;
    }
    return this;
  ...
  @override
  PointDrawText transformByHorizontalScale(Offset stationary, double scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
  ...
  @override
  PointDrawText transformByVerticalScale(Offset stationary, double scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    points = groupControlPoints ?? points;
  ...
  @override
  PointDrawText transformByScale(Offset stationary, Offset scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    points = groupControlPoints ?? points;
  ...
  Widget getIncrementFontButton(){
    return ActionButton(
      mode, false, displayWidget: const UpFontSizeIcon(widthSize: 28,), onPressed: (){
        fontSize++;
        notifyListeners();},
  ...
  Widget getDecrementFontButton(){
    return ActionButton(
      mode, false, displayWidget: const DownFontSizeIcon(widthSize: 28,), onPressed: (){
      if(fontSize > 0){
        fontSize--;
        notifyListeners();
      }},
  ...
  Widget getAlignLeftButton(){
    return ActionButton(
      mode, align == TextAlign.left, displayWidget: const Icon(Icons.align_horizontal_left, size: 16, color: Colors.white), onPressed:(){
  ...
  Widget getAlignCenterButton(){
    return ActionButton(
      mode, align == TextAlign.center, displayWidget: const Icon(Icons.align_horizontal_center, size: 16, color: Colors.white), onPressed:(){
  ...
  Widget getAlignRightButton(){
    return ActionButton(
      mode, align == TextAlign.right, displayWidget: const Icon(Icons.align_horizontal_right, size: 16, color: Colors.white), onPressed:(){
  ...
  Widget getToggleBoldButton(){
    return ActionButton(
      mode, weight == FontWeight.bold, displayWidget: const Icon(Icons.format_bold, size: 16, color: Colors.white), onPressed:(){
      ...
      child: Row(
        ...
        children: [
          Container(
              ...
              child: const Text("Font", style: TextStyle(fontSize:14, color: Colors.black))
          ),
          Expanded(
            child: PopupMenuButton<String>(
                itemBuilder: (context){
                  return [
                    for(String font in fontFamilies)
                      PopupMenuItem(
                        value: font,
                        child: Text(font, style: TextStyle(fontSize: 14, fontFamily: font)),
                      ),
                  ];
                },
              onSelected: (val){
              ...
              child: Material(
                  ...
                  child: Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 4.0),
                    child: Text(fontFamily ?? "Default font", style: const TextStyle(fontSize:14)),
                  ))
            ),
          ),
        ],
  ...
  Widget getToggleRoundedRectangleBorderButton(){
    return ActionButton(
      mode, roundedRectangleChatBoxBorder, displayWidget: const RoundedRectangleChatBoxIcon(widthSize: 24), onPressed:(){
      ...
      if(roundedRectangleChatBoxBorder){
        if(points.length == 1){
          ...
          gap = max(min(rect.width * 0.25 + radius! + _arrowGap, rect.width - radius! - _arrowGap), radius! + _arrowGap);
          rPoints = [rect.topLeft + Offset.fromDirection(pi / 2, radius!), rect.bottomRight - Offset(gap, 0)];
        }
      } else {
        points = [points.first];
  ...
  PointDrawText duplicate({Offset? center}){
    if(center != null){
      Offset displacement = center - boundingRect.center;
      return PointDrawText.from(this, displacement: displacement, key: ObjectKey("Text:"+generateAutoID()));
    } else {
      return PointDrawText.from(this, key: ObjectKey("Text:"+generateAutoID()));
    }
  }
...
class PointDrawPathOp extends PointDrawPath{
  ...
  PointDrawPathOp.fromDocument(
      DocumentSnapshot<Map<String, dynamic>> snapshot, this.firstPathObject, this.secondPathObject, this.op, {required ObjectKey key}
      ) : super.fromDocument(snapshot, mode: EditingMode.pathOp, key: key);

  PointDrawPathOp(this.firstPathObject, this.secondPathObject, {this.op = PathOperation.union, required ObjectKey key}) : super(mode: EditingMode.pathOp, key: key){
  ...
  PointDrawPathOp.from(PointDrawPathOp object, {this.firstPathObject, this.secondPathObject, this.op = PathOperation.union, Offset displacement = const Offset(5, 5), required ObjectKey key}) : super.from(object, displacement: displacement, mode: EditingMode.pathOp, key: key){
  ...
  void toObject(Map<String, dynamic> data, {parsePoints = true}){
    super.toObject(data, parsePoints: false);
    EditingMode firstPathObjectMode = getEditingMode(data[firstPathObjectKey][editingModeKey]);
    EditingMode secondPathObjectMode = getEditingMode(data[secondPathObjectKey][editingModeKey]);
  ...
  Path getPath(){
    Path path = Path();
    if(isInitialized){
      path = Path.combine(op, firstPathObject!.getPath(), secondPathObject!.getPath());
    }
    return path;
  }
  ...
  @override
  PointDrawPathOp transformByTranslate(double dx, double dy, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    super.transformByTranslate(dx, dy, zoomTransform, groupControlPoints: groupControlPoints);
    firstPathObject?.transformByTranslate(dx, dy, zoomTransform, groupControlPoints: groupControlPoints);
    secondPathObject?.transformByTranslate(dx, dy, zoomTransform, groupControlPoints: groupControlPoints);
    return this;
  }
  ...
  PointDrawObject transformByRotate(Offset center, double angle, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    ...
    firstPathObject?.transformByRotate(center, angle, zoomTransform, groupControlPoints: groupControlPoints);
    secondPathObject?.transformByRotate(center, angle, zoomTransform, groupControlPoints: groupControlPoints);
    return this;
  }
  ...
  PointDrawObject transformByHorizontalScale(Offset stationary, double scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    ...
    firstPathObject?.transformByHorizontalScale(stationary, scaleFactor, zoomTransform, groupControlPoints: groupControlPoints);
    secondPathObject?.transformByHorizontalScale(stationary, scaleFactor, zoomTransform, groupControlPoints: groupControlPoints);
    return this;
  }
  ...
  PointDrawObject transformByVerticalScale(Offset stationary, double scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    ...
    firstPathObject?.transformByVerticalScale(stationary, scaleFactor, zoomTransform, groupControlPoints: groupControlPoints);
    secondPathObject?.transformByVerticalScale(stationary, scaleFactor, zoomTransform, groupControlPoints: groupControlPoints);
    return this;
  }
  ...
  PointDrawObject transformByScale(Offset stationary, Offset scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    ...
    firstPathObject?.transformByScale(stationary, scaleFactor, zoomTransform, groupControlPoints: groupControlPoints);
    secondPathObject?.transformByScale(stationary, scaleFactor, zoomTransform, groupControlPoints: groupControlPoints);
    return this;
  }
  ...
  PointDrawPathOp duplicate({Offset? center}){
    if(center != null){
      Offset displacement = center - boundingRect.center;
      return PointDrawPathOp.from(this, displacement: displacement, key: ObjectKey("Path op:" + generateAutoID()));
    } else {
      return PointDrawPathOp.from(this, key: ObjectKey("Path op:" + generateAutoID()));
    }
  }
