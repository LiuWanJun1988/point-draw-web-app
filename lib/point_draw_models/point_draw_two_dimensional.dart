...
abstract class PointDrawTwoDimensional extends PointDrawPath {
  ...
  PointDrawTwoDimensional({this.glowRadius = 0.0, EditingMode mode = EditingMode.path, required ObjectKey key}) : super(mode: mode, key: key){
  ...
  PointDrawTwoDimensional.from(PointDrawTwoDimensional object, {EditingMode mode = EditingMode.path, Offset displacement = const Offset(5, 5), required ObjectKey key}) : super.from(object, displacement: displacement, mode: mode, key : key){
  ...
  double get glowRatio => boundingRect.isEmpty ? 0.0 : glowRadius / (boundingRect.longestSide / 2);
  ...
  void drawGlow(Canvas canvas, Path path, {Matrix4? zoomTransform}){
    Paint glowPaint = Paint()
      ..color = fPaint.color
      ..maskFilter = MaskFilter.blur(blurStyle, Shadow.convertRadiusToSigma(glowRadius));
  ...
  Widget getToggleGlowButton(){
    return ActionButton(
      mode,
      glowRadius != 0.0,
      displayWidget: const ShadowIcon(widthSize: 28,),
      onPressed: (){
        if(glowRadius == 0.0){
          glowRadius = 5.0;
        } else {
...
abstract class PointDrawStraightEdgedShape extends PointDrawTwoDimensional {
  ...
  PointDrawStraightEdgedShape.fromDocument(
    ...
    supplementaryPropertiesModifiers.addAll([getToggleRegulariseButton, getToggleRoundCornersButton]);
  }
  ...
  PointDrawStraightEdgedShape.from(PointDrawStraightEdgedShape object, {EditingMode mode = EditingMode.shape, this.roundCorners = false, Offset displacement = const Offset(5, 5), required ObjectKey key}) : super.from(object, displacement: displacement, mode: mode, key: key){
  ...
  Offset getRoundingCenter(int index, Path polygon){
    ...
    Offset centerArrow = Offset.fromDirection(((start - corner).direction + (end - corner).direction) / 2, 1);
    Offset center;
    if(polygon.contains(corner + centerArrow)){
      center = corner + centerArrow * roundingRadius / cos(getBasicAngle((endAngle - startAngle) / 2));
    } else {
      center = corner - centerArrow * roundingRadius / cos(getBasicAngle((endAngle - startAngle) / 2));
    }
    return center;
  ...
  Path getRoundedCorner(Offset start, Offset corner, Offset end, Offset roundingCenter){
    double startAngle = (corner - start).direction + pi / 2;
    double endAngle = (corner - end).direction - pi / 2;
    Rect arcRect = Rect.fromCenter(center: roundingCenter, width: roundingRadius * 2, height: roundingRadius * 2);
    Offset startOffset = roundingCenter + Offset.fromDirection(startAngle, roundingRadius);
...
class PointDrawPolygon extends PointDrawStraightEdgedShape {
  ...
  PointDrawPolygon({this.sides, mode = EditingMode.polygon, required ObjectKey key}) : super(mode: mode, key: key){
    enableDeleteControlPoint = true;
  }

  PointDrawPolygon.from(PointDrawPolygon object, {EditingMode mode = EditingMode.polygon, this.sides, Offset displacement = const Offset(5, 5), required ObjectKey key}) : super.from(object, displacement: displacement, mode: mode, key: key){
    sides = object.sides;
    enableDeleteControlPoint = true;
  }
    ...
    if(points.length >= 3){
      if(roundCorners){
        ...
        for(int i = 0; i < points.length; i++){
          ...
          Offset end = (points[(i + 1) % points.length] + points[(i + 2) % points.length]) / 2;
          polygon.extendWithPath(
            getRoundedCorner(start, points[(i + 1) % points.length], end, getRoundingCenter((i + 1) % points.length, interior)),
            Offset.zero
          );
        }
      } else {
  ...
  void autoInitializeControlPoints(){
    ...
    if(mode == EditingMode.triangle && points.length == 2){
      ...
      points.add((points.first + points.last) / 2 + Offset.fromDirection(displacement.direction + pi / 2, displacement.distance / 2));
    }
    if(mode == EditingMode.pentagon && points.length == 2){
      ...
      points = [
        ...
        rect.center + Offset.fromDirection(displacement.direction - 2 * pi / 5, width),
        rect.center + Offset.fromDirection(displacement.direction - 4 * pi / 5, width),
        rect.center + Offset.fromDirection(displacement.direction - 6 * pi / 5, width),
        rect.center + Offset.fromDirection(displacement.direction - 8 * pi / 5, width),
      ];
    }
    if(mode == EditingMode.polygon && points.length == 2){
      ...
      points.insert(1, (points.first + points.last) / 2 + Offset.fromDirection(direction, gap));
    }
    super.autoInitializeControlPoints();
  }
  ...
  void addControlPoint(Offset newPoint){
    for(int i = 0; i < points.length; i++){
      ...
      if(Rect.fromCenter(center: (firstPoint + secondPoint) * 0.5, width: max((firstPoint.dx - secondPoint.dx).abs(), 10), height: max((firstPoint.dy - secondPoint.dy).abs(), 10)).contains(newPoint)){
        points.insert(i + 1, newPoint);
        rPoints.insert(i + 1, getRoundingCenter(i + 1, Path()
          ..addPolygon(points, true)));
        return;
      }
    }
  ...
  PointDrawPolygon duplicate({Offset? center}){
    if(center != null){
      ...
      return PointDrawPolygon.from(this, displacement: displacement, mode: mode, key: ObjectKey("Polygon:"+generateAutoID()));
    } else {
      return PointDrawPolygon.from(this, mode: mode, key: ObjectKey("Polygon:"+generateAutoID()));
    }
  }

  @override
  String toString() => toProper(mode.name.substring(0, min(mode.name.length, 10)));
...
class PointDrawRoundedRectangle extends PointDrawPolygon {
  ...
  PointDrawRoundedRectangle.from(PointDrawRoundedRectangle object, {this.radius = 15.0, Offset displacement = const Offset(5, 5), EditingMode mode = EditingMode.roundedRectangle, required ObjectKey key}) : super.from(object, displacement: displacement, mode: mode, key: key){
  ...
  Path getPath(){
    if(points.length == 2 && rPoints.isNotEmpty){
      ...
      return Path()
        ..addRRect(
            RRect.fromRectAndRadius(Rect.fromPoints(points.first, points.last),
  ...
  @override
  PointDrawRoundedRectangle moveControlPoint(Offset newPosition, int index, {Map<String, dynamic>? args}){
  ...
  @override
  PointDrawRoundedRectangle updateRDSCPWhenCPMoved(Matrix4 zoomTransform, {Map<String, dynamic> args = const {}}){
  ...
  @override
  PointDrawRoundedRectangle moveRestrictedControlPoint(Offset localPosition, int index, {Map<String, dynamic>? args}){
  ...
  @override
  PointDrawRoundedRectangle transformByRotate(Offset center, double angle, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
  ...
  @override
  PointDrawRoundedRectangle flipVertical(Matrix4 zoomTransform, {Offset? center}){
    return this;
  }

  @override
  PointDrawRoundedRectangle flipHorizontal(Matrix4 zoomTransform, {Offset? center}){
    return this;
  }
  ...
  PointDrawRoundedRectangle duplicate({Offset? center}){
    if(center != null){
      Offset displacement = center - boundingRect.center;
      return PointDrawRoundedRectangle.from(this, displacement: displacement, key: ObjectKey("RoundedRectangle:"+generateAutoID()));
    } else {
      return PointDrawRoundedRectangle.from(this, key: ObjectKey("RoundedRectangle:"+generateAutoID()));
    }
  }
...
class PointDrawConic extends PointDrawTwoDimensional{
  ...
  PointDrawConic.fromDocument(DocumentSnapshot<Map<String, dynamic>> snapshot, {
    this.width = 100,
    this.height = 100,
  ...
  PointDrawConic.from(PointDrawConic object, {this.width = 100, this.height = 100, this.axialLock = false, Offset displacement = const Offset(5, 5), required ObjectKey key}) : super.from(object, mode: EditingMode.conic, displacement: displacement, key: key){
    width = object.width;
  ...
  @override
  bool get isInitialized => points.isNotEmpty && dPoints.isNotEmpty && rPoints.isNotEmpty;
  ...
  Path getPath(){
    Path conic = Path();
    if(points.length == 1){
      conic.addOval(Rect.fromCenter(center: points[0], width: width, height: height));
      return conic.transform(rotateZAbout((rPoints.first - points[0]).direction, points[0]).storage);
    }
    return conic;
  }
  ...
  Path getAnimatedPath(double ticker){
    ...
    if(points.length == 1){
      animatedConic.addOval(Rect.fromCenter(center: points[0], width: width, height: height));
      return animatedConic.transform(rotateZAbout((rPoints.first - points[0]).direction, points[0]).storage);
    }
  ...
  void autoInitializeControlPoints(){
    if(points.length == 1 && rPoints.isEmpty){
      ...
      rPoints = [points[0] + Offset.fromDirection(0, (rect.center - rect.bottomRight).distance)];
      dPoints = [rect.bottomRight];
    }
  }

  @override
  PointDrawConic moveControlPoint(Offset newPosition, int index, {Map<String, dynamic>? args}){
  ...
  @override
  PointDrawConic updateRDSCPWhenCPMoved(Matrix4 zoomTransform, {Map<String, dynamic>? args}){
    Rect rect = Rect.fromCenter(center: points[0], width: width, height: height);
    rPoints[0] = rect.center + Offset.fromDirection(_orientation, (rect.bottomRight - rect.center).distance);
  ...
  @override
  PointDrawConic moveRestrictedControlPoint(Offset localPosition, int index, {Map<String, dynamic>? args}){
    Rect rect = Rect.fromCenter(center: points[0], width: width, height: height);
    _orientation = (localPosition - rect.center).direction;
    rPoints[index] = rect.center + Offset.fromDirection(_orientation, (rect.center - rect.bottomRight).distance);
    return this;
  }

  @override
  PointDrawConic moveDataControlPoint(Offset newPosition, int index, {Map<String, dynamic>? args}){
    ...
    rPoints[0] = matrixApply(rotationMatrix, points[0] + Offset.fromDirection(0, (rect.bottomRight - rect.center).distance));
    return this;
  }

  @override
  PointDrawConic transformByRotate(Offset center, double angle, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    points = groupControlPoints ?? List<Offset>.generate(points.length, (ind) => rotate(points[ind], center, angle));
  ...
  @override
  PointDrawConic transformByHorizontalScale(Offset stationary, double scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    ...
    Rect rect = Rect.fromCenter(center: points[0], width: width, height: height);
    rPoints[0] = points[0] + Offset.fromDirection((Offset(stationary.dx + (rPoints[0].dx - stationary.dx) * scaleFactor, rPoints[0].dy) - points[0]).direction, (rect.center - rect.bottomRight).distance);
  ...
  @override
  PointDrawConic transformByVerticalScale(Offset stationary, double scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    ...
    Rect rect = Rect.fromCenter(center: points[0], width: width, height: height);
    rPoints[0] = points[0] + Offset.fromDirection((Offset(rPoints[0].dx, stationary.dy + (rPoints[0].dy - stationary.dy) * scaleFactor) - points[0]).direction, (rect.center - rect.bottomRight).distance);
  ...
  @override
  PointDrawConic transformByScale(Offset stationary, Offset scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    ...
    Rect rect = Rect.fromCenter(center: points[0], width: width, height: height);
    rPoints[0] = points[0] + Offset.fromDirection((Offset(stationary.dx + (rPoints[0].dx - stationary.dx) * scaleFactor.dx, stationary.dy + (rPoints[0].dy - stationary.dy) * scaleFactor.dy) - points[0]).direction, (rect.center - rect.bottomRight).distance);
  ...
  PointDrawConic duplicate({Offset? center}){
    if(center != null){
      Offset displacement = center - boundingRect.center;
      return PointDrawConic.from(this, displacement: displacement, key: ObjectKey("Conic:"+generateAutoID()));
    } else {
      return PointDrawConic.from(this, key: ObjectKey("Conic:"+generateAutoID()));
    }
  }
...
class PointDrawHeart extends PointDrawStraightEdgedShape{
  ...
  PointDrawHeart.fromDocument(
      DocumentSnapshot<Map<String, dynamic>> snapshot, {
        required ObjectKey key
      }) : super.fromDocument(snapshot, mode: EditingMode.heart, key: key);

  PointDrawHeart({this.orientation = 0.0, required ObjectKey key}) : super(mode: EditingMode.heart, key: key);

  PointDrawHeart.from(PointDrawHeart object, {Offset displacement = const Offset(5, 5), required ObjectKey key}) : super.from(object, displacement: displacement, mode: EditingMode.heart, key: key){
  ...
  void toObject(Map<String, dynamic> data, {bool parsePoints = true}){
    ...
    width = data[conicWidthKey];
    height = data[conicHeightKey];
    curvatureOffset = Offset(data[curvatureOffsetXKey], data[curvatureOffsetYKey]);
  }
  ...
  Path getPath(){
    ...
    if(isInitialized){
      Rect rect = Rect.fromCenter(center: points.first, width: width, height: height);
      ...
      heartShapePath = heartShapePath.transform(rotateZAbout(orientation, rect.center).storage);
    }
  ...
  Path getAnimatedPath(double ticker){
    ...
    if(isInitialized){
      ...
      Rect rect = Rect.fromCenter(center: animatedPoints.first, width: width, height: height);
      Rect upperRect = rect.topLeft & Size(rect.width, rect.height * 0.6);
      Rect lowerRect = (rect.centerLeft + Offset(0, - rect.height * 0.2)) & Size(rect.width, rect.height * 0.7);
      Offset blControlPoint = animatedPoints.first + curvatureOffset;
      Offset brControlPoint = blControlPoint + Offset(2 * (rect.center.dx - blControlPoint.dx), 0);
      animatedHeart.addArc(Rect.fromPoints(upperRect.topLeft, upperRect.bottomCenter), pi, pi);
      animatedHeart.addArc(Rect.fromPoints(upperRect.topRight, upperRect.bottomCenter), pi, pi);
      animatedHeart.quadraticBezierTo(brControlPoint.dx, brControlPoint.dy, lowerRect.bottomCenter.dx, lowerRect.bottomCenter.dy);
      animatedHeart.quadraticBezierTo(blControlPoint.dx, blControlPoint.dy, lowerRect.topLeft.dx, lowerRect.topLeft.dy);
      animatedHeart = animatedHeart.transform(rotateZAbout(orientation, rect.center).storage);
    }
  ...
  void autoInitializeControlPoints(){
    if(points.isNotEmpty){
      rPoints.add(Rect.fromCenter(center: points.first, width: width, height: height).topLeft + Offset(0, height * 0.7));
  ...
  @override
  PointDrawHeart moveControlPoint(Offset newPosition, int index, {Map<String, dynamic>? args}){
    args!["translate"] = newPosition - points.first;
    super.moveControlPoint(newPosition, index, args: args);
    updateRDSCPWhenCPMoved(args["zoom_transform"], args: args);
    return this;
  ...
  @override
  PointDrawHeart updateRDSCPWhenCPMoved(Matrix4 zoomTransform, {Map<String, dynamic> args = const {}}){
    rPoints[0] = rotate(points.first + curvatureOffset, points.first, orientation);
    super.updateRDSCPWhenCPMoved(zoomTransform, args: args);
    return this;
  }

  @override
  PointDrawHeart moveRestrictedControlPoint(Offset localPosition, int index, {Map<String, dynamic>? args}){
    Rect rect = Rect.fromCenter(center: points.first, width: width, height: height);
    Offset pivot = rect.topLeft + Offset(0, height * 0.3);
    Offset reorientatedOffset = points.first + Offset.fromDirection((localPosition - points.first).direction - orientation, (localPosition - points.first).distance);
    if((localPosition - rect.center).distance < (rect.bottomLeft - rect.center).distance && (reorientatedOffset - pivot).dy / (reorientatedOffset - pivot).dx > (rect.bottomCenter - pivot).dy / (rect.bottomCenter - pivot).dx){
      rPoints[index] = localPosition;
      curvatureOffset = rotate(rPoints[index], points.first, -orientation) - points.first;
    }
    return this;
  }

  @override
  PointDrawHeart transformByRotate(Offset center, double angle, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    points = groupControlPoints ?? List<Offset>.generate(points.length, (ind) => rotate(points[ind], center, angle));
    rPoints = List<Offset>.generate(rPoints.length, (ind) => rotate(rPoints[ind], center, angle));
    orientation += angle;
    shaderParam = shaderParam?.transformByRotate(center, angle);
  ...
  @override
  PointDrawHeart transformByHorizontalScale(Offset stationary, double scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    points = groupControlPoints ?? points;
    width = width * scaleFactor;
    rPoints.first = points.first + Offset.fromDirection((rPoints.first - points.first).direction, (rPoints.first - points.first).distance) * scaleFactor;
    curvatureOffset = rotate(rPoints.first, points.first, -orientation) - points.first;
    shaderParam = shaderParam?.transformByHorizontalScale(stationary, scaleFactor);
    fPaint.shader = fPaint.shader != null ? shaderParam?.build(boundingRect: boundingRect, zoomTransform: zoomTransform) : null;
    return this;
  }
  ...
  @override
  PointDrawHeart transformByVerticalScale(Offset stationary, double scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    points = groupControlPoints ?? points;
    height = height * scaleFactor;
    rPoints.first = points.first + Offset.fromDirection((rPoints.first - points.first).direction, (rPoints.first - points.first).distance) * scaleFactor;
    curvatureOffset = rotate(rPoints.first, points.first, -orientation) - points.first;
  ...
  @override
  PointDrawHeart transformByScale(Offset stationary, Offset scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    points = groupControlPoints ?? points;
    width = width * scaleFactor.dx;
    height = height * scaleFactor.dy;
    Offset scalingOffset = Offset.fromDirection((rPoints.first - points.first).direction, (rPoints.first - points.first).distance);
    rPoints.first = points.first + Offset(scalingOffset.dx * scaleFactor.dx, scalingOffset.dy * scaleFactor.dy);
    curvatureOffset = rotate(rPoints.first, points.first, -orientation) - points.first;
    shaderParam = shaderParam?.transformByScale(stationary, scaleFactor);
    fPaint.shader = fPaint.shader != null ? shaderParam?.build(boundingRect: boundingRect, zoomTransform: zoomTransform) : null;
    return this;
  ...
  PointDrawHeart duplicate({Offset? center}){
    ...
    if(center != null){
      Offset displacement = center - boundingRect.center;
      return PointDrawHeart.from(this, displacement: displacement, key: ObjectKey("Heart:"+generateAutoID()));
    } else {
      return PointDrawHeart.from(this, key: ObjectKey("Heart:"+generateAutoID()));
    }
  }
...
class PointDrawArrow extends PointDrawStraightEdgedShape{
  ...
  PointDrawArrow({this.directionalGap = 0.0, this.orthogonalGap = 0.0, required ObjectKey key}) : super(mode: EditingMode.arrow, key: key);

  PointDrawArrow.from(PointDrawArrow object, {this.directionalGap = 0.0, this.orthogonalGap = 0.0, Offset displacement = const Offset(5, 5), required ObjectKey key}) : super.from(object, displacement: displacement, mode: EditingMode.arrow, key: key){
  ...
  Path getPath(){
    ...
    if(points.length == 2 && rPoints.length == 2){
      ...
      Offset lerpPoint = points[0] + Offset.fromDirection((points[1] - points[0]).direction, directionalGap);
  ...
  Path getAnimatedPath(double ticker){
    ...
    if(points.length == 2 && rPoints.length == 2){
      ...
      Offset displacement = rPoints[0] - rPoints[1];
      Offset lerpPoint = animatedPoints[0] + Offset.fromDirection((animatedPoints[1] - animatedPoints[0]).direction, directionalGap);
      double normal = displacement.direction;
      double dist = displacement.distance;
  ...
  void autoInitializeControlPoints(){
    if(points.length == 2 && rPoints.isEmpty){
      ...
      orthogonalGap = dist;
      rPoints = [lerpPoint + Offset.fromDirection(normal, dist), lerpPoint + Offset.fromDirection(normal, dist / 2)];
    }
  }

  @override
  PointDrawArrow moveControlPoint(Offset newPosition, int index, {Map<String, dynamic>? args}){
    super.moveControlPoint(newPosition, index, args: args);
  ...
  @override
  PointDrawArrow updateRDSCPWhenCPMoved(Matrix4 zoomTransform, {Map<String, dynamic> args = const {}}){
    super.updateRDSCPWhenCPMoved(zoomTransform);
    double direction = (points[1] - points[0]).direction;
    double normal = direction + pi / 2;
    double dist = (rPoints[0] - rPoints[1]).distance;
    Offset lerpPoint = points[0] + Offset.fromDirection(direction, directionalGap);
  ...
  @override
  PointDrawArrow moveRestrictedControlPoint(Offset localPosition, int index, {Map<String, dynamic>? args}){
    if(args!["restriction_index"] == 0){
      ...
      rPoints[1] = points[0] + Offset.fromDirection(direction, directionalGap) + Offset.fromDirection(direction + pi / 2, orthogonalGap - dist);
      rPoints[index] = points[0] + Offset.fromDirection(direction, directionalGap) + Offset.fromDirection(direction + pi / 2, orthogonalGap);
    } else {
      double direction = (points[1] - points[0]).direction;
      double dist = distanceFromLine(localPosition, direction + pi / 2, rPoints[0]);
      rPoints[index] = points[0] + Offset.fromDirection(direction, directionalGap) + Offset.fromDirection(direction + pi / 2, orthogonalGap - dist);
    }
    return this;
  }

  @override
  PointDrawArrow transformByHorizontalScale(Offset stationary, double scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    points = groupControlPoints ?? List<Offset>.generate(points.length, (ind) => Offset(stationary.dx + (points[ind].dx - stationary.dx) * scaleFactor, points[ind].dy));
    double direction = (points[1] - points[0]).direction;
    double dist = (rPoints[0] - rPoints[1]).distance;
    rPoints[0] = points[0] + Offset.fromDirection(direction, directionalGap) + Offset.fromDirection(direction + pi / 2, orthogonalGap);
    rPoints[1] = points[0] + Offset.fromDirection(direction, directionalGap) + Offset.fromDirection(direction + pi / 2, orthogonalGap - dist);
    return this;
  }

  @override
  PointDrawArrow transformByVerticalScale(Offset stationary, double scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    points = groupControlPoints ?? List<Offset>.generate(points.length, (ind) => Offset(points[ind].dx, stationary.dy + (points[ind].dy - stationary.dy) * scaleFactor));
    double direction = (points[1] - points[0]).direction;
    double dist = (rPoints[0] - rPoints[1]).distance;
    rPoints[0] = points[0] + Offset.fromDirection(direction, directionalGap) + Offset.fromDirection(direction + pi / 2, orthogonalGap);
  ...
  @override
  PointDrawArrow transformByScale(Offset stationary, Offset scaleFactor, Matrix4 zoomTransform, {List<Offset>? groupControlPoints}){
    points = groupControlPoints ?? List<Offset>.generate(points.length, (ind) => Offset(stationary.dx + (points[ind].dx - stationary.dx) * scaleFactor.dx, stationary.dy + (points[ind].dy - stationary.dy) * scaleFactor.dy));
    double direction = (points[1] - points[0]).direction;
    double dist = (rPoints[0] - rPoints[1]).distance;
    rPoints[0] = points[0] + Offset.fromDirection(direction, directionalGap) + Offset.fromDirection(direction + pi / 2, orthogonalGap);
  ...
  PointDrawArrow duplicate({Offset? center}){
    if(center != null){
      Offset displacement = center - boundingRect.center;
      return PointDrawArrow.from(this, displacement: displacement, key: ObjectKey("Arrow:"+generateAutoID()));
    } else {
      return PointDrawArrow.from(this, key: ObjectKey("Arrow:"+generateAutoID()));
    }
  }
...
class PointDrawStar extends PointDrawStraightEdgedShape{
  ...
  PointDrawStar({this.radius, this.corners = 5, required ObjectKey key}) : super(mode: EditingMode.star, key: key){
    supplementaryPropertiesModifiers.add(getCornersModifier);
  }

  PointDrawStar.from(PointDrawStar object, {this.radius, Offset displacement = const Offset(5, 5), required ObjectKey key}) : super.from(object, displacement: displacement, mode: EditingMode.star, key: key){
    radius = object.radius;
  ...
  @override
  void autoInitializeControlPoints(){
    radius ??= (points.first - points.last).distance * sin(pi / 10) / sin(7 * pi / 10);
    rPoints.add(points.first + Offset.fromDirection((points.last - points.first).direction + (pi / corners), radius!));
  }
  ...
  Path getPath(){
    ...
    if(points.length == 2){
      ...
      for(int i = 0; i < corners; i++){
        vertices.add(points.first + Offset.fromDirection(startAngleOuter + i * (2 * pi / corners), outerRadius));
        vertices.add(points.first + Offset.fromDirection(startAngleInner + i * (2 * pi / corners), radius!));
      }
  ...
  Path getAnimatedPath(double ticker){
    ...
    if(points.length == 2){
      ...
      double startAngleInner = startAngleOuter + pi / corners;
      for(int i = 0; i < corners; i++){
        vertices.add(center + Offset.fromDirection(startAngleOuter + i * (2 * pi / corners), outerRadius));
        vertices.add(center + Offset.fromDirection(startAngleInner + i * (2 * pi / corners), innerRadius));
      }
  ...
  @override
  PointDrawStar moveControlPoint(Offset newPosition, int index, {Map<String, dynamic>? args}){
    super.moveControlPoint(newPosition, index, args: args);
    updateRDSCPWhenCPMoved(args!["zoom_transform"], args: args);
    return this;
  }
  ...
  @override
  PointDrawStar updateRDSCPWhenCPMoved(Matrix4 zoomTransform, {Map<String, dynamic> args = const {}}){
    rPoints[0] = points.first + Offset.fromDirection((points.last - points.first).direction + (pi / corners), radius!);
    super.updateRDSCPWhenCPMoved(zoomTransform);
    return this;
  }

  @override
  PointDrawStar moveRestrictedControlPoint(Offset localPosition, int index, {Map<String, dynamic>? args}){
    radius = min((localPosition - points.first).distance, (points.first - points.last).distance);
    rPoints[0] = points.first + Offset.fromDirection((points.last - points.first).direction + (pi / corners), radius!);
    return this;
  }
  ...
  PointDrawStar duplicate({Offset? center}){
    if(center != null){
      Offset displacement = center - boundingRect.center;
      return PointDrawStar.from(this, displacement: displacement, key: ObjectKey("Star:"+generateAutoID()));
    } else {
      return PointDrawStar.from(this, key: ObjectKey("Star:"+generateAutoID()));
    }
  }
...
class PointDrawLeaf extends PointDrawTwoDimensional{
  ...
  PointDrawLeaf({this.symmetric = false, this.orthSymmetric = false, required ObjectKey key}) : super(mode: EditingMode.leaf, key: key){
    supplementaryPropertiesModifiers.addAll([
      getToggleSymmetricButton,
      getToggleOrthoSymmetricButton,
    ]);
  ...
  PointDrawLeaf.from(PointDrawLeaf object, {this.symmetric = false, this.orthSymmetric = false, Offset displacement = const Offset(5, 5), required ObjectKey key}) : super.from(object, displacement: displacement, mode: EditingMode.leaf, key: key){
  ...
  Path getPath(){
    ...
    Path leaf = Path();
    if((points.length == 3 && symmetric && orthSymmetric) || (points.length == 4 && symmetric) || points.length == 6){
      ...
      if(symmetric && orthSymmetric){
        ...
        cubicCP2 = points[3];
        double cp3Direction = 2 * (points[0] - points[1]).direction - (cubicCP2 - points[1]).direction;
        double cp1Distance = (cubicCP1 - points[0]).distance;
        double cp4Direction = 2 * (points[1] - points[0]).direction - (cubicCP1 - points[0]).direction;
      ...
      leaf.cubicTo(cubicCP1.dx, cubicCP1.dy, cubicCP2.dx, cubicCP2.dy, points[1].dx, points[1].dy);
      leaf.cubicTo(cubicCP3.dx, cubicCP3.dy, cubicCP4.dx, cubicCP4.dy, points[0].dx, points[0].dy);
      leaf.close();
    }
    return leaf;
  }
  ...
  Path getAnimatedPath(double ticker){
    ...
    if((points.length == 3 && symmetric && orthSymmetric) || (points.length == 4 && symmetric) || points.length == 6){
      ...
      Offset cubicCP2, cubicCP3, cubicCP4;
      if(symmetric && orthSymmetric){
        Offset center = Rect.fromPoints(animatedPoints[0], animatedPoints[1]).center;
        double cp1Direction = (cubicCP1 - center).direction;
        double cp1Distance = (cubicCP1 - center).distance;
        cubicCP2 = center + Offset.fromDirection(cp1Direction + pi / 2, cp1Distance);
        cubicCP3 = center + Offset.fromDirection(cp1Direction + pi, cp1Distance);
        ...
        double cp3Direction = 2 * (animatedPoints[0] - animatedPoints[1]).direction - (cubicCP2 - animatedPoints[1]).direction;
        double cp1Distance = (cubicCP1 - animatedPoints[0]).distance;
        double cp4Direction = 2 * (animatedPoints[1] - animatedPoints[0]).direction - (cubicCP1 - animatedPoints[0]).direction;
        double cp2Distance = (cubicCP2 - animatedPoints[1]).distance;
        cubicCP3 = animatedPoints[1] + Offset.fromDirection(cp3Direction, cp2Distance);
        cubicCP4 = animatedPoints[0] + Offset.fromDirection(cp4Direction, cp1Distance);
      } else {
        cubicCP2 = animatedPoints[3];
      ...
      animatedLeaf.cubicTo(cubicCP1.dx, cubicCP1.dy, cubicCP2.dx, cubicCP2.dy, animatedPoints[1].dx, animatedPoints[1].dy);
      animatedLeaf.cubicTo(cubicCP3.dx, cubicCP3.dy, cubicCP4.dx, cubicCP4.dy, animatedPoints[0].dx, animatedPoints[0].dy);
      animatedLeaf.close();
    }
    return animatedLeaf;
  }
  ...
  PointDrawLeaf duplicate({Offset? center}){
    if( center != null ){
      Offset displacement = center - boundingRect.center;
      return PointDrawLeaf.from(this, displacement: displacement, key: ObjectKey("Leaf:"+generateAutoID()));
    } else {
      return PointDrawLeaf.from(this, key: ObjectKey("Leaf:"+generateAutoID()));
    }
  }
...
class PointDrawBlob extends PointDrawTwoDimensional{
  ...
  PointDrawBlob.fromDocument(
      DocumentSnapshot<Map<String, dynamic>> snapshot, {
        required ObjectKey key, EditingMode mode = EditingMode.blob,
      }) : super.fromDocument(snapshot, mode: mode, key: key){
    enableDeleteControlPoint = true;
  }

  PointDrawBlob({required ObjectKey key, EditingMode mode = EditingMode.blob}) : super(mode: mode, key: key){
    enableDeleteControlPoint = true;
  }

  PointDrawBlob.from(PointDrawBlob object, {Offset displacement = const Offset(5, 5), required ObjectKey key, EditingMode mode = EditingMode.blob}) : super.from(object, displacement: displacement, mode: mode, key: key){
    enableDeleteControlPoint = true;
  }
  ...
  void addControlPoint(Offset newPoint){
    for(int i =0; i < points.length - 1; i++){
      if(Rect.fromCenter(center: (points[i] + points[i + 1]) * 0.5, width: max((points[i].dx - points[i+1].dx).abs(), 10), height: max((points[i].dy - points[i+1].dy).abs(), 10)).contains(newPoint)){
        points.insert(i + 1, newPoint);
        return;
      }
    }
  ...
  @override
  PointDrawBlob moveControlPoint(Offset newPosition, int index, {Map<String, dynamic>? args}){
    super.moveControlPoint(newPosition, index, args: args);
    return this;
  }
  ...
  PointDrawBlob duplicate({Offset? center}){
    if(center != null){
      Offset displacement = center - boundingRect.center;
      return PointDrawBlob.from(this, displacement: displacement, key: ObjectKey("Blob:" + generateAutoID()));
    } else {
      return PointDrawBlob.from(this, key: ObjectKey("Blob:" + generateAutoID()));
    }
  }
